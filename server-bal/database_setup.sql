...
INSERT INTO projects (project_name, category_id, allocated_budget, state, province, ministry, view_details, status) 
VALUES (
    ...
    'A comprehensive project to improve digital infrastructure across the Western Province',
    'ACTIVE'
) ON CONFLICT DO NOTHING;

-- Success message
SELECT 'Database schema setup completed successfully! All tables, indexes, and sample data have been created.' AS status;

-- IMPORTANT: Disable Row Level Security for REST API Access (Development Only)
-- This must be run to enable API access from our Ballerina backend
...
SELECT 'All tables created and Row Level Security disabled for development. Backend API access is now enabled.' AS final_status;
