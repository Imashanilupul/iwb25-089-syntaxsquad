...
CREATE TABLE IF NOT EXISTS transactions (
    id SERIAL PRIMARY KEY,
    entity_type VARCHAR(50) NOT NULL,                  -- e.g. 'user', 'vote', 'petition'
    entity_id INTEGER NOT NULL,                        -- ID of the specific entity
    tx_hash VARCHAR(100) NOT NULL,                     -- blockchain transaction hash
    status VARCHAR(20) NOT NULL DEFAULT 'pending'      -- pending, confirmed, failed
        CHECK (status IN ('pending', 'confirmed', 'failed')),
    change_data JSONB,                                 -- store any change details
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
...
-- Add likes and dislikes columns to reports table immediately after creation
ALTER TABLE reports 
...
-- Create user_votes table for tracking report likes/dislikes
...
-- Create report_likes table (alternative approach for better performance)
...
-- Create report_dislikes table (alternative approach for better performance)
...
-- Indexes for report voting tables
...
AND table_name IN (
    ...
    'user_votes', 'report_likes', 'report_dislikes'
)
ORDER BY table_name;

-- Create triggers to update report like/dislike counts automatically
...
-- Update existing reports to have initial like/dislike counts
UPDATE reports SET 
